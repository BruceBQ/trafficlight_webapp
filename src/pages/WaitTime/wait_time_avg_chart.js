...
class WaitTimeAvgChart extends React.Component {
  constructor(props) {
    super(props)
    this.state = {
  ...
  componentDidUpdate(prevProps) {
    if (
      ...
      JSON.stringify(prevProps.waitAvg) !== JSON.stringify(this.props.waitAvg)
    ) {
      this.createAxis()
    }
  }
  ...
  createScaleX0 = () => {
    ...
    const { waitAvg } = this.props
    return d3
      .scaleBand()
      .rangeRound([0, width])
      .paddingInner(0.1)
      .domain(waitAvg.map((data) => new Date(data.time)))
  }
  ...
  renderGroups = () => {
    ...
    return waitAvg.map((avg, index1) => {
      const rectangles = keys.map((key) => ({ key, value: avg[key] }))
      return (
        <g key={index1} className="bar" transform={`translate(${scaleX0(new Date(avg.time)) || 0},0)`}>
...
const mapStateToProps = ({ waitTime }) => {
  return {
    waitAvg: waitTime.data.waittimeavg,
  }
}

export default connect(mapStateToProps)(WithSize(withStyles(styles)(WaitTimeAvgChart)))
